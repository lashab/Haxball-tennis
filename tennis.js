(function() {
  ...
  room.onGameStart = (byPlayer) => {
    ...
    players.forEach(element => {
      if (element.id !== 0 && !sets.hasOwnProperty(element.id)) {
        sets[element.id] = {
          win: 0
        }
      }
  ...
  room.onPlayerChat = (player, message) => {
    if (message == '!chat_off' && player.admin) {
      _chat = false;
    }
    
    if (message == '!chat_on' && player.admin) {
      _chat = true;
    }
  ...
  room.onTeamGoal = (team) => {
    ...
    if (playerBlue.length && playerRed.length && typeof playerBlue !== 'undefined' && playerRed !== 'undefined' && !winner) {
      room.sendChat(`${playerRed[0].name} (${red}) - ${playerBlue[0].name} (${blue})`);
    }

    if (winner) {
      const playerWinner = players.filter((player) => {
        return player.team == winner;
      });

      const playerLoser = players.filter((player) => {
        return player.team == winner == 1 ? 2 : 1;
      });
